...
module.exports = async function condloHandler(req, res) {
  ...
  if (req.method === 'GET') {
    const st = await readStore(redis);
    // sinal de vida do robô: última execução do checkCondLo registrada no notifications_log
    let ultimaVerificacao = null;
    try {
      const lraw = await redis.get('notifications_log');
      const log = Array.isArray(lraw) ? lraw : (lraw ? JSON.parse(lraw) : []);
      const e = log.find(function (x) { return x && x.scope === 'condlo'; });
      if (e) ultimaVerificacao = e.at || null;
    } catch (e) {}
    return res.status(200).json({ ok: true, licencas: st.licencas, conds: st.conds, solTemplate: st.solTemplate || null, ultimaVerificacao: ultimaVerificacao, me: { role: role, canEdit: canEdit } });
  }
  ...
  if (req.method === 'POST') {
    ...
    // modelo padrão do e-mail de solicitação (opcional; mantém o atual se não vier no corpo)
    let solTemplate = cur.solTemplate || null;
    if (body.solTemplate && typeof body.solTemplate === 'object') {
      solTemplate = {
        assunto: String(body.solTemplate.assunto || '').slice(0, 200),
        corpo: String(body.solTemplate.corpo || '').slice(0, 4000),
      };
      if (!solTemplate.corpo) solTemplate = null;
    }
    try { await redis.set(STORE_KEY, JSON.stringify({ licencas: licencas, conds: conds, solTemplate: solTemplate })); } catch (e) { return res.status(500).json({ error: 'Erro ao salvar: ' + e.message }); }
